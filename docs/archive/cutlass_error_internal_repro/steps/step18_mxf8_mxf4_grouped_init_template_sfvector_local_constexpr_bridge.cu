...
// Step18: Copy of step15 (FAILS) but reintroduce a local constexpr bridge:
//   constexpr int kSF = SFVectorSize;
// and use kSF everywhere (including as the Sm1xxBlockScaledConfig template arg).
...
int run_once(int num_groups, int M, int N, int K) {
  ...
  auto sfa_layout0 = cutlass::detail::Sm1xxBlockScaledConfig<kSF>::tile_atom_to_shape_SFA(
      cute::make_shape(M, N, K, 1));
  auto sfb_layout0 = cutlass::detail::Sm1xxBlockScaledConfig<kSF>::tile_atom_to_shape_SFB(
      cute::make_shape(M, N, K, 1));
  ...
  std::printf("[step18] num_groups=%d M=%d N=%d K=%d (SF=%d) can_implement=%s\n",
              num_groups, M, N, K, kSF, cutlass_status(can));
  ...
  std::printf("[step18] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
