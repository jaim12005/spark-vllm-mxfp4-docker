...
int run_once(int num_groups, int M, int N, int K) {
  ...
  // Only change vs step13: go back to int64 grouped shapes.
  using ProblemShape =
  ...
  std::printf("[step14] num_groups=%d M=%d N=%d K=%d can_implement=%s\n",
  ...
  std::printf("[step14] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
