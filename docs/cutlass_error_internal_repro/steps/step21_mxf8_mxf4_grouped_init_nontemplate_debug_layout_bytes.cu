...
template <typename T>
static void dump_bytes(char const* label, T const& obj, int max_bytes = 64) {
  auto const* p = reinterpret_cast<unsigned char const*>(&obj);
  int n = (int)sizeof(T);
  if (n > max_bytes) n = max_bytes;
  std::printf("%s sizeof=%zu align=%zu bytes[0:%d]=", label, sizeof(T), alignof(T), n);
  for (int i = 0; i < n; ++i) {
...
// Step21: like step14 (SUCCESS), but adds the same debug dumps as step20.
int run_once(int num_groups, int M, int N, int K) {
  ...
  using LayoutSFAVal = std::remove_pointer_t<LayoutSFA>;
  using LayoutSFBVal = std::remove_pointer_t<LayoutSFB>;
  using StrideAVal = std::remove_pointer_t<StrideA>;
  using StrideBVal = std::remove_pointer_t<StrideB>;
  using StrideDVal = std::remove_pointer_t<StrideD>;

  static_assert(std::is_trivially_copyable_v<LayoutSFAVal>, "LayoutSFAVal must be trivially copyable");
  static_assert(std::is_trivially_copyable_v<LayoutSFBVal>, "LayoutSFBVal must be trivially copyable");
  static_assert(std::is_trivially_copyable_v<StrideAVal>, "StrideAVal must be trivially copyable");
  static_assert(std::is_trivially_copyable_v<StrideBVal>, "StrideBVal must be trivially copyable");
  static_assert(std::is_trivially_copyable_v<StrideDVal>, "StrideDVal must be trivially copyable");
  ...
  std::printf("[step21] SF=%d cosize(SFA)=%lld cosize(SFB)=%lld sfa_bytes=%zu sfb_bytes=%zu\n",
              SFVectorSize,
              (long long)cute::cosize(sfa_layout0),
              (long long)cute::cosize(sfb_layout0),
              sfa_bytes, sfb_bytes);

  dump_bytes("decltype(sfa_layout0)", sfa_layout0);
  dump_bytes("LayoutSFAVal{}       ", LayoutSFAVal{});

  auto stride_a0 = cutlass::make_cute_packed_stride(
      StrideAVal{}, cute::Shape<int, int, int>{M, K, 1});
  auto stride_b0 = cutlass::make_cute_packed_stride(
      StrideBVal{}, cute::Shape<int, int, int>{N, K, 1});
  auto stride_d0 = cutlass::make_cute_packed_stride(
      StrideDVal{}, cute::Shape<int, int, int>{M, N, 1});
  dump_bytes("StrideAVal packed    ", stride_a0);
  dump_bytes("StrideBVal packed    ", stride_b0);
  dump_bytes("StrideDVal packed    ", stride_d0);
  ...
  auto* stride_A = cuda_malloc_elems<StrideAVal>(num_groups);
  auto* stride_B = cuda_malloc_elems<StrideBVal>(num_groups);
  auto* stride_D = cuda_malloc_elems<StrideDVal>(num_groups);
  auto* layout_SFA = cuda_malloc_elems<LayoutSFAVal>(num_groups);
  auto* layout_SFB = cuda_malloc_elems<LayoutSFBVal>(num_groups);
  ...
  for (int i = 0; i < num_groups; ++i) {
    ...
    h_B_ptr[i] = B;
    h_D_ptr[i] = D;
    h_stride_A[i] = stride_a0;
    h_stride_B[i] = stride_b0;
    h_stride_D[i] = stride_d0;
  ...
  dump_bytes("h_layout_SFA[0]      ", h_layout_SFA[0]);
  dump_bytes("h_layout_SFB[0]      ", h_layout_SFB[0]);
  ...
  std::printf("[step21] num_groups=%d M=%d N=%d K=%d can_implement=%s\n",
              num_groups, M, N, K, cutlass_status(can));
  ...
  std::printf("[step21] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
...
int main(int argc, char** argv) {
  ...
  return run_once(num_groups, M, N, K);
}
