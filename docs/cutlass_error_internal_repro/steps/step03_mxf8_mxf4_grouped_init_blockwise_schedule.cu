...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  // Schedule experiment: try SM120 "blockwise" schedule instead of pingpong.
  // This mirrors what FlashInfer uses in the known-working FP8-groupwise path.
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ArchTag, OperatorClass, ElementA, LayoutA*, AlignmentA, ElementB, LayoutB*, AlignmentB,
          ElementAccumulator, TileShape_MNK, ClusterShape_MNK, StageCount,
          cutlass::gemm::collective::KernelScheduleSm120Blockwise>::CollectiveOp;
  ...
  std::printf("[step03] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step03] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
