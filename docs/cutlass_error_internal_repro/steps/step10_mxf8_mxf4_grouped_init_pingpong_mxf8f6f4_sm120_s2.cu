...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  // Critical change vs step01: use a ptr-array schedule tag that is explicitly
  // derived from KernelScheduleMxf8f6f4Sm120, so CUTLASS can validate/deduce
  // the correct SF vector width for MX_F4F6F8 instruction selection.
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ArchTag, OperatorClass, ElementA, LayoutA*, AlignmentA, ElementB, LayoutB*, AlignmentB,
          ElementAccumulator, TileShape_MNK, ClusterShape_MNK, StageCount,
          cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpongMxf8f6f4Sm120<2>>::CollectiveOp;
  ...
  std::printf("[step10] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step10] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
