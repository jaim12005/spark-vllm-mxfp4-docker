...
// Step15: Start from step14 (SUCCESS) and re-introduce ONLY the template SFVectorSize
// structure used in step01/10/11/12, while keeping M/N/K as int args.
...
int run_once(int num_groups, int M, int N, int K) {
  ...
  std::printf("[step15] num_groups=%d M=%d N=%d K=%d can_implement=%s\n",
  ...
  std::printf("[step15] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
