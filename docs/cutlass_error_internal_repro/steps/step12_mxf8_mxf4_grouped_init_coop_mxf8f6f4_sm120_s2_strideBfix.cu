...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ...
          cutlass::gemm::KernelPtrArrayTmaWarpSpecializedCooperativeMxf8f6f4Sm120<2>>::CollectiveOp;
  ...
  for (int i = 0; i < num_groups; ++i) {
    ...
    // IMPORTANT: B is (K, N) in GEMM, and ColumnMajor means stride = (1, K).
    // Using Shape(N, K) here incorrectly produces (K, 1) and can break TMA descriptors.
    h_stride_B[i] = cutlass::make_cute_packed_stride(
        std::remove_pointer_t<StrideB>{},
        cute::Shape<int, int, int>{(int)K, (int)N, 1});
  ...
  std::printf("[step12] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step12] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
