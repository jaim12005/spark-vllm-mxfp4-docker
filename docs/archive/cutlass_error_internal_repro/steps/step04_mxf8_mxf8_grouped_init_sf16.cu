...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  return 2;
#else
  // MXF8 × MXF8 with SFVectorSize=16 (CUTLASS requires this for MX_F4F6F8 variants).
  ...
  std::printf("[step04] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step04] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
...
int main(int argc, char** argv) {
  ...
  return run_once<16>(num_groups, M, N, K);
}
