...
// Step16: Copy of step15 (which FAILS) with one change:
// pass a valid host_problem_shapes pointer in GroupProblemShape.
...
int run_once(int num_groups, int M, int N, int K) {
  ...
  typename Gemm::Arguments args{
      ...
      // Difference vs step15: provide host_problem_shapes
      {num_groups, problem_sizes, h_shapes.data()},
      {A_ptr,
  ...
  std::printf("[step16] num_groups=%d M=%d N=%d K=%d can_implement=%s\n",
  ...
  std::printf("[step16] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
