...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  // Schedule experiment: use the SM120 blockscaled instruction tag rather than the generic ptr-array tag.
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ArchTag, OperatorClass, ElementA, LayoutA*, AlignmentA, ElementB, LayoutB*, AlignmentB,
          ElementAccumulator, TileShape_MNK, ClusterShape_MNK, StageCount,
          cutlass::gemm::KernelTmaWarpSpecializedPingpongMxf8f6f4Sm120>::CollectiveOp;
  ...
  auto* B = cuda_malloc_elems<ElementInputB>(static_cast<size_t>(N * K));
  ...
  std::printf("[step05] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step05] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
...
int main(int argc, char** argv) {
  ...
  // NOTE: This step is explicitly about schedule-tag-driven SfVectorSize deduction, so we keep
  // SFVectorSize=32 and let the builder validate/deduce.
