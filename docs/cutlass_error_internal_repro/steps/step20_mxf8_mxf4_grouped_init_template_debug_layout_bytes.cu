...
// Step20: Like step15 (FAILS), but dumps layout/stride bytes to detect UB/mismatches.
...
int run_once(int num_groups, int M, int N, int K) {
  ...
  std::printf("[step20] SF=%d cosize(SFA)=%lld cosize(SFB)=%lld sfa_bytes=%zu sfb_bytes=%zu\n",
              SFVectorSize,
  ...
  std::printf("[step20] num_groups=%d M=%d N=%d K=%d can_implement=%s\n",
  ...
  std::printf("[step20] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
