...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  return 2;
#else
  // MXF8 × MXF8 (no FP4) to isolate whether FP4 operand B is the trigger.
  ...
  std::printf("[step02] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step02] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
