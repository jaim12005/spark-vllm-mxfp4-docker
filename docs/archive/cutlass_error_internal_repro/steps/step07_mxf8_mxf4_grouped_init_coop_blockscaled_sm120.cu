...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ...
          cutlass::gemm::KernelPtrArrayTmaWarpSpecializedCooperativeBlockScaledSm120>::CollectiveOp;
  ...
  std::printf("[step07] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step07] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
