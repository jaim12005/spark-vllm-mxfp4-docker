...
// Step22: keep run_once templated, but move *all CUTLASS kernel type definitions*
// out of the function template to namespace scope. Hypothesis: nesting kernel types
// inside a function template triggers initialize(): Error Internal on SM12x.
namespace step22_types {
...
static_assert(std::is_trivially_copyable_v<LayoutSFAVal>);
static_assert(std::is_trivially_copyable_v<LayoutSFBVal>);
static_assert(std::is_trivially_copyable_v<StrideAVal>);
static_assert(std::is_trivially_copyable_v<StrideBVal>);
...
int run_once(int num_groups, int M, int N, int K) {
  ...
  std::printf("[step22] SF=%d can_implement=%s\n", SFVectorSize, cutlass_status(can));
  ...
  std::printf("[step22] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
