...
int run_once(int num_groups, int64_t M, int64_t N, int64_t K) {
  ...
  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ...
          cutlass::gemm::KernelPtrArrayTmaWarpSpecializedPingpongBlockScaledSm120<2>>::CollectiveOp;
  ...
  std::printf("[step08] num_groups=%d M=%lld N=%lld K=%lld can_implement=%s\n",
  ...
  std::printf("[step08] initialize=%s (ws_bytes=%zu)\n", cutlass_status(init), ws_bytes);
